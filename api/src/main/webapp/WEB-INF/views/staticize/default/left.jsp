    ...
    <div class="mb0 r2 f14 mr15 mt20 p20 BGFFF mb20 col-md-mr0">
        ...
        <ul class="panel-body b0 p0 ml10">
            ...
            <c:forEach var="f" items="${moduleList}" varStatus="status">
                <c:if test="${needStaticizes.indexOf(',interface,')>=0}">
                    <li><a target="_self" class="p5 cursor C${f.id}_interface"
                           href="${f.id}-interfaceList-1.html">${f.name}</a></li>
                </c:if>
                <c:if test="${needStaticizes.indexOf(',article,')>=0}">
                    <li><a target="_self" class="p5 cursor C${f.id}_article""
                        href="${f.id}-articleList--1.html">${f.name}</a></li>
                </c:if>
                <c:if test="${needStaticizes.indexOf(',dictionary,')>=0}">
                    <li><a target="_self" class="p5 cursor C${f.id}_dictionary"
                           href="${f.id}-dictionaryList-1.html">${f.name}</a></li>
                </c:if>
            </c:forEach>
        </ul>
        <%} else { %>
        ...
        <div class="no-radius b0 mt0">
            <div class="no-radius p10" data-parent="#accordion">
                <div class="a cursor adorn-bl-3 pl10 no_unl " data-toggle="collapse" data-parent="#accordion"
                     href="#panel${f.id}">
                        ${f.name}
                </div>
            </div>
